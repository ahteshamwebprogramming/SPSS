function Submit() {
    BlockUI();
    if (validate_AddPage.form()) {
        var formData = new FormData();
        formData.append("Page", $("#PageForm").find($('[name="Page"]')).val());
                ...
                if (data != null) {
                    if (data.length > 0) {
                        if (data == "InsertSuccess") {
                            ...
                            $.alert({ title: 'Error!', content: 'This Page already exists. Please try another!', });
                        }
                            ...
                            $.alert({ title: 'Error!', content: 'The Page is mandatory!', });
...
var validate_AddPage;

validate_AddPage = $("#PageForm").validate({
    rules: {
        Page: "required"
    },
    messages: {
        Page: "Page cannot be left blank"
    }
});
...
function FetchTable() {
    $.ajax({
        ...
        url: '/BuildingForms/FetchPage',
        //data: formData,
        cache: false,
        dataType: "html",
        success: function (data, textStatus, jqXHR) {
            $("#div_PageTable").html(data);
        },
        error: function (result) {
...
function Edit(Id, Value) {
    $('#Submit').html("Update");
    $('#hfId').val(Id);
    $('#PageForm').find($('[name="Page"]')).val(Value);
}
