function Submit() {
    BlockUI();
    if (validate_AddGroup.form()) {
        var formData = new FormData();
        formData.append("Group", $("#GroupForm").find($('[name="Group"]')).val());
                ...
                if (data != null) {
                    if (data.length > 0) {
                        if (data == "InsertSuccess") {
                            ...
                            $.alert({ title: 'Error!', content: 'This Group already exists. Please try another!', });
                        }
                            ...
                            $.alert({ title: 'Error!', content: 'The Group is mandatory!', });
...
var validate_AddGroup;

validate_AddGroup = $("#GroupForm").validate({
    rules: {
        Group: "required"
    },
    messages: {
        Group: "Group cannot be left blank"
    }
});
...
function FetchTable() {
    $.ajax({
        ...
        url: '/UserMaster/FetchGroup',
        //data: formData,
        cache: false,
        dataType: "html",
        success: function (data, textStatus, jqXHR) {
            $("#div_GroupTable").html(data);
        },
        error: function (result) {
...
function Edit(Id, Value) {
    $('#Submit').html("Update");
    $('#hfId').val(Id);
    $('#GroupForm').find($('[name="Group"]')).val(Value);
}
