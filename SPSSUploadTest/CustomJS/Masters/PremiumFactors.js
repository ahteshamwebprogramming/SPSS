function Submit() {
    BlockUI();
    if (validate_AddPremiumFactors.form()) {
        var formData = new FormData();
        formData.append("PremiumFactors", $("#PremiumFactorsForm").find($('[name="PremiumFactors"]')).val());
                ...
                if (data != null) {
                    if (data.length > 0) {
                        if (data == "InsertSuccess") {
                            ...
                            $.alert({ title: 'Error!', content: 'This PremiumFactors already exists. Please try another!', });
                        }
                            ...
                            $.alert({ title: 'Error!', content: 'The PremiumFactors is mandatory!', });
...
var validate_AddPremiumFactors;

validate_AddPremiumFactors = $("#PremiumFactorsForm").validate({
    rules: {
        PremiumFactors: "required"
    },
    messages: {
        PremiumFactors: "PremiumFactors cannot be left blank"
    }
});
...
function FetchTable() {
    $.ajax({
        ...
        url: '/Masters/FetchPremiumFactors',
        //data: formData,
        cache: false,
        dataType: "html",
        success: function (data, textStatus, jqXHR) {
            $("#div_PremiumFactorsTable").html(data);
        },
        error: function (result) {
...
function Edit(Id, Value) {
    $('#Submit').html("Update");
    $('#hfId').val(Id);
    $('#PremiumFactorsForm').find($('[name="PremiumFactors"]')).val(Value);
}
...
function Reset() {
    $('#Submit').html("Add");
    $('#hfId').val(0);
    $("#PremiumFactorsForm").find($('[name="PremiumFactors"]')).val("");
}