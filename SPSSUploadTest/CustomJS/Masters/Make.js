function Submit() {
    BlockUI();
    if (validate_AddMake.form()) {
        var formData = new FormData();
        formData.append("Make", $("#MakeForm").find($('[name="Make"]')).val());
                ...
                if (data != null) {
                    if (data.length > 0) {
                        if (data == "InsertSuccess") {
                            ...
                            $.alert({ title: 'Error!', content: 'This Make already exists. Please try another!', });
                        }
                            ...
                            $.alert({ title: 'Error!', content: 'The Make is mandatory!', });
...
var validate_AddMake;

validate_AddMake = $("#MakeForm").validate({
    rules: {
        Make: "required"
    },
    messages: {
        Make: "Make cannot be left blank"
    }
});
...
function FetchTable() {
    $.ajax({
        ...
        url: '/Masters/FetchMake',
        //data: formData,
        cache: false,
        dataType: "html",
        success: function (data, textStatus, jqXHR) {
            $("#div_MakeTable").html(data);
        },
        error: function (result) {
...
function Edit(Id, Value) {
    $('#Submit').html("Update");
    $('#hfId').val(Id);
    $('#MakeForm').find($('[name="Make"]')).val(Value);
}
