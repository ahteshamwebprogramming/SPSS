function Submit() {
    BlockUI();
    if (validate_AddProduct.form()) {
        var formData = new FormData();
        formData.append("Product", $("#ProductForm").find($('[name="Product"]')).val());
                ...
                if (data != null) {
                    if (data.length > 0) {
                        if (data == "InsertSuccess") {
                            ...
                            $.alert({ title: 'Error!', content: 'This Product already exists. Please try another!', });
                        }
                            ...
                            $.alert({ title: 'Error!', content: 'The Product is mandatory!', });
...
var validate_AddProduct;

validate_AddProduct = $("#ProductForm").validate({
    rules: {
        Product: "required"
    },
    messages: {
        Product: "Product cannot be left blank"
    }
});
...
function FetchTable() {
    $.ajax({
        ...
        url: '/Masters/FetchProduct',
        //data: formData,
        cache: false,
        dataType: "html",
        success: function (data, textStatus, jqXHR) {
            $("#div_ProductTable").html(data);
        },
        error: function (result) {
...
function Edit(Id, Value) {
    $('#Submit').html("Update");
    $('#hfId').val(Id);
    $('#ProductForm').find($('[name="Product"]')).val(Value);
}
...
function Reset() {
    $('#Submit').html("Add");
    $('#hfId').val(0);
    $("#ProductForm").find($('[name="Product"]')).val("");
}