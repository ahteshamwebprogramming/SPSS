function Submit() {
    BlockUI();
    if (validate_AddSaleMethod.form()) {
        var formData = new FormData();
        formData.append("SaleMethod", $("#SaleMethodForm").find($('[name="SaleMethod"]')).val());
                ...
                if (data != null) {
                    if (data.length > 0) {
                        if (data == "InsertSuccess") {
                            ...
                            $.alert({ title: 'Error!', content: 'This SaleMethod already exists. Please try another!', });
                        }
                            ...
                            $.alert({ title: 'Error!', content: 'The SaleMethod is mandatory!', });
...
var validate_AddSaleMethod;

validate_AddSaleMethod = $("#SaleMethodForm").validate({
    rules: {
        SaleMethod: "required"
    },
    messages: {
        SaleMethod: "SaleMethod cannot be left blank"
    }
});
...
function FetchTable() {
    $.ajax({
        ...
        url: '/Masters/FetchSaleMethod',
        //data: formData,
        cache: false,
        dataType: "html",
        success: function (data, textStatus, jqXHR) {
            $("#div_SaleMethodTable").html(data);
        },
        error: function (result) {
...
function Edit(Id, Value) {
    $('#Submit').html("Update");
    $('#hfId').val(Id);
    $('#SaleMethodForm').find($('[name="SaleMethod"]')).val(Value);
}
...
function Reset() {
    $('#Submit').html("Add");
    $('#hfId').val(0);
    $("#SaleMethodForm").find($('[name="SaleMethod"]')).val("");
}