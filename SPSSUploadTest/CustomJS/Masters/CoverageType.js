function Submit() {
    BlockUI();
    if (validate_AddCoverageType.form()) {
        var formData = new FormData();
        formData.append("CoverageType", $("#CoverageTypeForm").find($('[name="CoverageType"]')).val());
                ...
                if (data != null) {
                    if (data.length > 0) {
                        if (data == "InsertSuccess") {
                            ...
                            $.alert({ title: 'Error!', content: 'This CoverageType already exists. Please try another!', });
                        }
                            ...
                            $.alert({ title: 'Error!', content: 'The CoverageType is mandatory!', });
...
var validate_AddCoverageType;

validate_AddCoverageType = $("#CoverageTypeForm").validate({
    rules: {
        CoverageType: "required"
    },
    messages: {
        CoverageType: "CoverageType cannot be left blank"
    }
});
...
function FetchTable() {
    $.ajax({
        ...
        url: '/Masters/FetchCoverageType',
        //data: formData,
        cache: false,
        dataType: "html",
        success: function (data, textStatus, jqXHR) {
            $("#div_CoverageTypeTable").html(data);
        },
        error: function (result) {
...
function Edit(Id, Value) {
    $('#Submit').html("Update");
    $('#hfId').val(Id);
    $('#CoverageTypeForm').find($('[name="CoverageType"]')).val(Value);
}
...
function Reset() {
    $('#Submit').html("Add");
    $('#hfId').val(0);
    $("#CoverageTypeForm").find($('[name="CoverageType"]')).val("");
}