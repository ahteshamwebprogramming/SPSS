function Submit() {
    BlockUI();
    if (validate_AddExcessDeductible.form()) {
        var formData = new FormData();
        formData.append("ExcessDeductible", $("#ExcessDeductibleForm").find($('[name="ExcessDeductible"]')).val());
        ...
        $.ajax({
            type: 'POST',
            url: "/Masters/AddExcessDeductible",
            data: formData,
            cache: false,
            contentType: false,
            processData: false,
                ...
                if (data != null) {
                    if (data.length > 0) {
                        if (data == "InsertSuccess") {
                            ...
                            $.alert({ title: 'Error!', content: 'This ExcessDeductible already exists. Please try another!', });
                        }
                            ...
                            $.alert({ title: 'Error!', content: 'The ExcessDeductible is mandatory!', });
...
var validate_AddExcessDeductible;

validate_AddExcessDeductible = $("#ExcessDeductibleForm").validate({
    rules: {
        ExcessDeductible: "required"
    },
    messages: {
        ExcessDeductible: "ExcessDeductible cannot be left blank"
    }
});
...
function FetchTable() {
    $.ajax({
        ...
        url: '/Masters/FetchExcessDeductible',
        //data: formData,
        cache: false,
        dataType: "html",
        success: function (data, textStatus, jqXHR) {
            $("#div_ExcessDeductibleTable").html(data);
        },
        error: function (result) {
...
function Edit(Id, Value) {
    $('#Submit').html("Update");
    $('#hfId').val(Id);
    $('#ExcessDeductibleForm').find($('[name="ExcessDeductible"]')).val(Value);
}
...
function Reset() {
    $('#Submit').html("Add");
    $('#hfId').val(0);
    $("#ExcessDeductibleForm").find($('[name="ExcessDeductible"]')).val("");
}