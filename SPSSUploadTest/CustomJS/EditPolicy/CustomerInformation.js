function CustomerInformationInitialise() {
    ...
    $('#Master_CIN_Search').parent().parent().find('[class="select2-selection__rendered"]').html(data.CIN);
    $('#hfCIN').val(data.CIN);

    $("#CustomerInformation").find($('[name="FirstName"]')).val(data.FirstName);
    $("#CustomerInformation").find($('[name="MiddleName"]')).val(data.MiddleName);
    $("#CustomerInformation").find($('[name="LastName"]')).val(data.LastName);
    $("#CustomerInformation").find($('[name="FullName"]')).val(data.FullName);
    $("#CustomerInformation").find($('[name="EmailId"]')).val(data.EmailId);
    $("#CustomerInformation").find($('[name="CompanyName"]')).val(data.CompanyName);
    $("#CustomerInformation").find($('[name="IDNumber"]')).val(data.IDNumber);
    $("#CustomerInformation").find($('[name="MobileNumber"]')).val(data.MobileNumber);
    $("#CustomerInformation").find($('[name="AlternateMobileNumber"]')).val(data.AlternateMobileNumber);
    $("#CustomerInformation").find($('[name="POBox"]')).val(data.CPOBox);
    $("#CustomerInformation").find($('[name="Address"]')).val(data.Address);

    $("#CustomerInformation").find($('[name="IDExpiry"]')).val(moment(new Date(parseInt(data.IDExpiry.replace(/[^0-9 +]/g, '')))).format('DD/MM/YYYY'));
    $("#CustomerInformation").find($('[name="DateOfBirth"]')).val(moment(new Date(parseInt(data.DateOfBirth.replace(/[^0-9 +]/g, '')))).format('DD/MM/YYYY'));

    FetchTitle().then(() => {
        $("#CustomerInformation").find($('[name="Title"]')).dropdown("set selected", data.Title);
    });
    FetchGender().then(() => {
        $("#CustomerInformation").find($('[name="Gender"]')).dropdown("set selected", data.Gender);
    });
    FetchIDType().then(() => {
        $("#CustomerInformation").find($('[name="IDType"]')).dropdown("set selected", data.IDType);
    });
    FetchEmirate().then(() => {
        $("#CustomerInformation").find($('[name="Emirate"]')).dropdown("set selected", data.Emirate);
    });
    FetchNationality().then(() => {
        $("#CustomerInformation").find($('[name="Nationality"]')).dropdown("set selected", data.Nationality);
    });
}
